  ...
  <div class="label-container">
    ...
    >
      ...
      <el-table-column align="center" label="是否推荐">
        <template slot-scope="{ row }">
          <el-switch
            @change="updateRecommend(row)"
            v-model="row.recommend"
            active-color="#13ce66"
            inactive-color="#ff4949"
            :inactive-value="recommendData.inactiveValue"
            :active-value="recommendData.activeValue"
          ></el-switch>
        </template>
      </el-table-column>
      <el-table-column
...
<script>
import { getList, doDelete, doEdit, doDeleteBatch } from '@/api/label'
...
export default {
  ...
  methods: {
    ...
    updateRecommend(row) {
      // return
      doEdit(row).then((res) => {
        if (res != null && res.code == 20000 && row.recommend == '1') {
          this.$message({
            message: '已推荐！',
            type: 'success',
          })
        } else if (res != null && res.code == 20000 && row.recommend == '0') {
          this.$message({
            message: '已取消推荐！',
            type: 'success',
          })
